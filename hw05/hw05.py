...
def make_withdraw(balance, password):
    ...
    def withdraw(money, testword):
        ...
        if balance < money and testword == password:
            return'Insufficient funds'
        elif balance >= money and testword == password:
            balance -= money
            return balance
        elif testword != password:
            count += 1    
            if count <= 3:
                wrong_password.append(testword)
...
def merge(incr_a, incr_b):
    ...
    while next_a is not None or next_b is not None:
        if next_a is None or next_b is not None and next_a > next_b:
            yield next_b
